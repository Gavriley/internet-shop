class Category < ActiveRecord::Base
	before_save -> { self.parent_id = 0 if self.parent_id.nil? }
	...
	def set_parent_to_children
		self.children.each { |child| child.update_column(:parent_id, 0) }
	end	

	def check_children_categories
		set_parent_to_children if self.parent_id != 0 && children.exists?
	end	
